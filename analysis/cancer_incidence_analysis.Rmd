...
```{r}
#Number of cases by cancer site where incidence is over 100 cases.
cancer_incidence_border %>% 
  filter(cancer_site != "All cancer types") %>% 
  filter(sex == "All") %>% 
...
As can be seen from the plot, cancer incidence in the NHS Borders is low when compared to other parts of Scotland as a whole. This is possibly due to population density, however further investigation is required to create a fuller picture and confirm this.
...
cancer_incidence_border %>% 
  ...
  aes(x = year, y = crude_rate) +
   #geom_point(aes(y = crude_rate_lower95pc_confidence_interval), color = "orange") +
  #geom_point(aes(y = crude_rate_upper95pc_confidence_interval), color = "blue") +
  geom_line() +
  geom_point() +
  theme_minimal() +
  labs(
...
2. Non-melanoma skin cancer is extremely high for both genders.
3. The crude rate of Trachea, Bronchus and Lung Cancers is higher for males, however more recently the rate for females was low in 1994 and has increased to the point where the rate is now closely lagging behind males.


Data Sources:- 

Cancer Incidence - Cancer incidence data - https://www.opendata.nhs.scot/dataset/annual-cancer-incidence
